...
### FIGURE 6 - comparison with the non-reversible variant sampeler
n <- 1000
B <- 300
N <- 50
set.seed(123)
figure6 <- sapply(c(3,10,20,50), function(K) {
  ...
  for(b in seq_len(B)){
    ...
    tmp <- capture.output(PNR <- nrMCmix(numeric(n),K = K, alpha = alpha,
                                         N = N-1,warm_up = 0, xi = 2 / K / (K-1),
    ...
    tmp <- capture.output(QNR <- nrMCmix(numeric(n),K = K, alpha = alpha,
                                         N = N-1,warm_up = 0, s = 1, 
                                         kernel = "partition", init = init,
                                         save_configurations = TRUE))
...
violin(figure6[2,1]$QNR,figure6[1,1]$PNR, by = 5)
violin(figure6[2,2]$QNR,figure6[1,2]$PNR, by = 5)
violin(figure6[2,3]$QNR,figure6[1,3]$PNR, by = 5)
violin(figure6[2,4]$QNR,figure6[1,4]$PNR, by = 5)
